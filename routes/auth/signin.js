...
router.post('/', async (req, res) => {
    ...
    if (selectUserResult[0] == null) {//id가 존재하지 않으면
        ...
        res.status(200).send(defaultRes.successFalse(statusCode.OK, resMessage.NOT_CORRECT_ID));
    } else {
        ...
        if (selectUserResult[0].password == hashedEnterPw.toString('base64')) {
            ...
            if (!refreshTokenUpdateResult) {
                res.status(200).send(defaultRes.successFalse(statusCode.OK, resMessage.ERROR_SAVE_REFRESHTOKEN));
            } else {
